...
fun supprimerDoublons(liste: List<Int>): MutableList<Int> {
    // Copie de la liste :
    return liste.toMutableList().withIndex().map { listOf(it.value, it.index) } . associate { it[0] to it[1] } .keys.toMutableList()
}

// in python : list(dict(map( (lambda x : list(reversed(x)) ), list(enumerate(a)) )).keys())
...
fun main() {
    ...
    //supprimerDoublons(listOf(5, 5, 1, 3, 1, 7, 9, 3))
}